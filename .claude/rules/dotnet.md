...
live in the Constitution and are not repeated here. This file holds what is specific to C#, .NET,
and the WPF platform. Project and machine facts — the target framework and SDK version, paths,
commands, operating system, workload installs — live in `techContext.md`. This file names no
version, so it does not need editing when the project moves to the next one.
