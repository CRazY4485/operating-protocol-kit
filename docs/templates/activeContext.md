...
Record step once the task is accepted.** Everything under its heading is read back verbatim by the
`SessionStart` hook, so it holds the plan itself and no commentary.
...
cleared session gets none of it. Every rule that refers to "the approved plan" — scope lock, the
review step, the owner-check step, a sub-agent's brief — therefore depends on the plan being a file
in this repository. Delete the section when no task is open; do not leave a finished plan behind.
