...
- **Per runtime**, the gate set and its pass condition are defined in that language's rule file — not here, and not invented per task. A runtime with no gate listed is a blocker I report; so is a language with no rule file in `.claude/rules/`, and code in that language is not written until one exists.
- **Documents are gated too.** `.claude/tools/check-docs.py` checks encoding, budgets, cross-references, Tier 1 completeness, the decision index and `.claude/settings.json`.
