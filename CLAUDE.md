...
A `SessionStart` hook reports the kit version, the Tier 1 files with their word counts, the git state, and any open plan before the first turn. That report is data, not a verdict, and a hook that cannot start is a non-blocking error, so I verify rather than assume:
...
**Continuity across `/clear`.** The session boundary here is `/clear`, not `/compact`, and they are not equivalent. Claude Code re-injects the project `CLAUDE.md`, unscoped rules, auto memory, the plan-mode plan and recently read files after a *compaction*; `/clear` starts a new conversation and returns none of it. What crosses a `/clear` is exactly this: files in the repository, `CLAUDE.md` with its unscoped rules, and what the `SessionStart` hook injects. Anything the next session must know is a file, or it does not exist. Auto memory is off in `.claude/settings.json` for the same reason — it survives `/clear`, but it lives outside the repository and outside git, is machine-local, and is invisible to the owner, so it cannot hold a record the owner is meant to audit.
...
- **Enforcement, in two layers.** `.claude/settings.json` denies destructive commands and secret-file reads and logs which instruction files loaded; it also runs the document gate before a commit. That layer **fails open**: Claude Code treats a hook it cannot start, or one that times out, as a non-blocking error and lets the call through, so it is a fast signal rather than a gate. The gate that **fails closed** is git's own `.githooks/pre-commit`, which refuses the commit even when the interpreter is missing and applies to every committer. I never weaken or route around either; a missing hook is a `backlog.md` entry, not an excuse.
